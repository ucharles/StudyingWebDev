...
- 객체들이 차이점보다 유사성이 많다
- 객체를 동일한 컬렉션으로 유지하고 싶을 때

해결책

- 필드는 문서 또는 문서의 하위 유형을 추적한다
- 애플리케이션에는 문서 유형별 코드 경로가 있거나, 하위 클래스(문서별 차이가 상당한 경우)가 존재한다(OOP)
...
덜 자주 사용하는 패턴들... 하지만 도움은 될 것이다.

- Approximation Pattern (근사 패턴)

  - 이 패턴은 클라이언트 측 애플리케이션에 있어야 한다(프론트엔드?)
  - 이 패턴은 MongoDB 뿐만 아니라, 다른 DB와 함께 사용할 수 있다.
  - 빅데이터 애플리케이션에 더 자주 나타남.
  - 일부 쓰기 작업을 수행하는 데 필요한 리소스를 줄이기 위해 사용됨.
  - 이 패턴은 결과를 생성하는 데 근사 함수를 사용함. (흔히 말하는 이차 함수에 접하는 일차 함수...)
  - 예를 들어 카운터를 증가시키는 경우. 요청이 들어올 때마다 DB의 값을 갱신하면 빠르게 과부하가 올 것이다. 요청마다 DB를 갱신하는 대신, 서버에서 값을 보관 & 갱신 후 DB에는 한번에 갱신한다.

  - 해결하고자 하는 것

    - 계산하기에 비싼 데이터
    - 데이터의 정밀함이 요구되지 않을 때 사용 가능

  - 해결법

    - 적은 쓰기를 수행한다. 하지만 높은 payload로. 애플리케이션은 정해진 수마다 작업을 수행할 수 있다. (10번 마다, 100번 마다 등)

  - 사용예

    - 웹페이지 카운터
    - 부정확한 공차가 있는 모든 카운터(유튜브 구독자수?)
    - 미터법 통계

  - 장점과 단점
    - 적은 쓰기 횟수
    - 동일한 문서를 업데이트 하는 동안 일어날 수 있는 충돌이 적다.
    - 통계적으로 유효한 숫자들(단위 시간 당 정확하지 않은 경우)

- Outlier Pattern

  - 평균치에서 크게 벗어나, 다른 대상들과 확연히 구분되는 표본 (수많은 파란 물고기 안의 빨간 물고기)
  - 가장 빈번한 사용사례를 효과적으로 처리할 수 있음(아웃라이어를 성공적으로 처리하는 동안)

  - 해결하고자 하는 문제
    - 몇 개의 문서가 이 문제를 야기할 것
    - 대다수의 쿼리에 부정적인 영향을 미침
  - 해결책
    - 대다수를 위한 작업의 구현
    - 아웃라이어 필드를 예외로 정의한다.
    - 아웃라이어만 별도로 애플리케이션 측에서 처리한다.
  - 사용예
    - 소셜 네트워크(수많은 팔로워를 가진 계정)
    - 인기 관련 애플리케이션, 유명인이 포함된 앱
  - 장점과 단점
    - 애플리케이션이 대부분의 사용 사례에 최적화 되어있음.
    - 아웃라이너를 애플리케이션에서 별도로 처리해야 함.
    - 집합을 꾸리는게 어려움.
    - 쿼리 작성을 어렵게 만듦. (하나의 균일한 작업에서 전체 데이터 세트를 처리할 수 있는..)
    - 클라이언트 측에서 처리하므로, 어떤 DB에서도 구현할 수 있다.
